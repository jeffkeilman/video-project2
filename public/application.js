...
var config = {
  apiOrigins: {
    production: 'https://jk-video-api.herokuapp.com',
...
var videoCRUD = ['selectVideo', 'infoButton', 'updateButton', 'deleteButton', 'addButton', 'playButton'];

var showAdd = function showAdd(event) {
...
var showAddPrefill = function showAddPrefill(data) {
  _crudButtonsDisabled(true);
...
var createVideoFailure = function createVideoFailure() {
  alert('danger', 'Oh no. We couldn\'t add a video. This is bad.');
};

var displayVideos = function displayVideos(data) {
...
var resetVideoComponents = function resetVideoComponents() {
  $('#videoArea').empty();
...
var changeMenuText = function changeMenuText(event) {
  $('#selectVideo').html('<div data-id="' + $(event.currentTarget).attr('data-id') + '">' + $(event.currentTarget).text() + ' <span class="caret"></span></div>');
};

var showInfoModal = function showInfoModal(data) {
  var video = data.video;
...
var deployVideo = function deployVideo() {
  $('#videoModal').modal();
};

var _removeVideo = function _removeVideo(videos, id) {
...
var updateVideos = function updateVideos(video) {
  for (var x = 0; x < store.user.videos.length; x++) {
...
var _crudButtonsDisabled = function _crudButtonsDisabled(option) {
  videoCRUD.forEach(function (button) {
    $('#' + button).prop('disabled', option);
  });
};

var _buildModalBody = function _buildModalBody(video) {
  $('#infoModalBody').html('<div class="row">' + '<div>Youtuber: ' + video.youtuber + '</div>' + '</div>' + '<div class="row">' + '<div>Description: ' + video.description + '</div>' + '</div>');
};

var _buildVideoMenuItem = function _buildVideoMenuItem(video) {
  _prettifyVideo(video);
  $('#selectVideoMenu').append('<li><a class="dropdown-item" href="#" data-id="' + video.id + '">' + video.title + '</a></li>');
};

var _buildVideo = function _buildVideo(video) {
  _prettifyVideo(video);
  var newRow = document.createElement('div');
  $(newRow).addClass('row').html('<div class="col-xs-6">' + video.title + '</div><div class="col-xs-6"><a href="' + video.url + '" target="_blank">' + video.url + '</div>');
  $('#videoArea').append(newRow);
};

var _clearAdd = function _clearAdd() {
  $('#titleText').val('');
  $('#urlText').val('');
  $('#youtuberText').val('');
  $('#descriptionText').val('');
...
var _prettifyVideo = function _prettifyVideo(video) {
...
module.exports = {
  ...
  createVideoFailure: createVideoFailure,
  displayVideos: displayVideos,
  indexFailure: indexFailure,
  resetVideoComponents: resetVideoComponents,
  changeMenuText: changeMenuText,
  showInfoModal: showInfoModal,
  showFailure: showFailure,
  noID: noID,
  showAddPrefill: showAddPrefill,
...
var alert = __webpack_require__(7);
var store = __webpack_require__(1);
var videoUI = __webpack_require__(3);

var programViews = ['intro', 'login', 'changePass', 'db'];

var programNav = ['nav-login', 'nav-videos', 'nav-changePass', 'nav-logout'];
...
var displayVideos = function displayVideos() {
  _navBarShift($('#nav-videos'));
  _flipViews($('#db'));
};

var displayChangePass = function displayChangePass() {
...
var signOffSuccess = function signOffSuccess() {
  store.user = null;
  restoreInitial();
};

var signOffFailure = function signOffFailure() {
...
var restoreInitial = function restoreInitial() {
  // default state
  _hideViews();
  _hideNav();
...
var showHidePassConf = function showHidePassConf() {
...
var _navBarShift = function _navBarShift(li) {
  $('.active').removeClass('active');
  li.addClass('active');
};

var _flipViews = function _flipViews(newView) {
  _hideViews();
  newView.show();
};

var _hideViews = function _hideViews() {
  programViews.forEach(function (view) {
    $('#' + view).hide();
  });
};

var _hideNav = function _hideNav() {
  programNav.forEach(function (nav) {
    $('#' + nav).hide();
  });
};

var _clearLogin = function _clearLogin(all, passwordMismatch) {
  $('#passwordText').val('');
...
var _clearChangePass = function _clearChangePass() {
  $('#newPassText').val('');
  $('#oldPassText').val('');
};
...
module.exports = {
  ...
  showHidePassConf: showHidePassConf,
  passwordMismatch: passwordMismatch,
  signOnSuccess: signOnSuccess,
  signOnFailure: signOnFailure,
  displayVideos: displayVideos,
  restoreInitial: restoreInitial,
  signOffSuccess: signOffSuccess,
  signOffFailure: signOffFailure,
...
var alert = function alert(type, message) {
  ...
  programInfo.show().delay(5000).fadeOut(function () {
...
var videoEvents = __webpack_require__(16);
...
var videoAPI = __webpack_require__(8);
var videoUI = __webpack_require__(3);
...
var _onLogout = function _onLogout(event) {
  ...
  api.logout(data).then(ui.signOffSuccess).catch(ui.signOffFailure);
};
...
var _login = function _login(data) {
  ...
  api.login(data).then(ui.signOnSuccess).then(_loadVideos).catch(ui.signOnFailure);
};

var _loadVideos = function _loadVideos() {
  videoAPI.loadVideos().then(videoUI.displayVideos).catch(videoUI.indexFailure);
};
...
var _onVideoSubmit = function _onVideoSubmit(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  data.user_id = store.user.id;

  api.create(data).then(ui.createVideo).catch(ui.createVideoFailure);
};

var _onInfoClick = function _onInfoClick() {
  var id = $('#selectVideo div').attr('data-id');
  if (!isNaN(id)) {
    api.show(id).then(ui.showInfoModal).catch(ui.showFailure);
  } else {
    ui.noID();
  }
};

var _onUpdateClick = function _onUpdateClick() {
  // Run a show to get info and prefill
  var id = $('#selectVideo div').attr('data-id');
  if (!isNaN(id)) {
    api.show(id).then(ui.showAddPrefill).catch(ui.showFailure);
  } else {
    ui.noID();
  }
};

var _onUpdateSubmit = function _onUpdateSubmit(event) {
  var data = getFormFields(document.getElementById('addVideoForm'));
  var updateCount = _cleanupVideo(data.video);
  ui.updateVideos(data.video);

  if (updateCount > 0) {
    api.update(data).then(ui.updateSuccess).catch(ui.updateFailure);
  } else {
    ui.noUpdate();
  }
};

var _onDeleteClick = function _onDeleteClick() {
  var id = $('#selectVideo div').attr('data-id');
  if (!isNaN(id)) {
    api.deleteVideo(id).then(ui.deleteSuccess).catch(ui.deleteFailure);
  } else {
    ui.noID();
  }
};

var _cleanupVideo = function _cleanupVideo(video) {
  var updateCount = 0;
  for (var key in video) {
...
var _onPlay = function _onPlay() {
  ...
  if (!isNaN(id)) {
    ...
    var vidId = splitUrl[splitUrl.length - 1];

    player = (0, _youtubePlayer2.default)('videoPlayer', {
      height: 350,
      width: 850
    });

    player.loadVideoById(vidId).then(ui.deployVideo());
  } else {
    ui.noID();
  }
};
...
var _sister = __webpack_require__(18);

var _sister2 = _interopRequireDefault(_sister);

var _loadYouTubeIframeApi = __webpack_require__(20);

var _loadYouTubeIframeApi2 = _interopRequireDefault(_loadYouTubeIframeApi);

var _YouTubePlayer = __webpack_require__(22);
...
var _loadScript = __webpack_require__(21);
